...
def sobel(img):
    ...
    col = img.shape[1]
    num_pixel = row*col
    si = np.sum(dst)/num_pixel

    # cv2.imshow('sobel', dst)
    # cv2.waitKey(1)
    # save_path = '/data/Disk_D/shaochuan/Project/IC9600-master/example/edge/cat.jpg'
...
def get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans, out_mode='torch', new=False):
    ...
    return im_patch, im_to_torch(im_patch) if out_mode in 'torch' else im_patch
