...
class SiamRPN(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.regress_adjust(temp), \
               F.conv2d(self.conv_cls2(x_f), self.cls1_kernel), x_f
        # new added end

    # layer-wise forward 10.20
    def layer_forward(self, x):
        for i in range(len(self.featureExtract)):
            x = self.featureExtract[i](x)
            if i == 15:
                return x
    ...
    def after_relu(self, x):
        # x_f = self.featureExtract(x)
        x_f = nn.ReLU(inplace=False)(x)
        return x_f
    # new added end
