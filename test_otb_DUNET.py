...
from run_attack import SiamRPN_init, DUNET_pure_adv, DUNET_pure_clean, DAA_DUNET
...
# new added 5.8
os.sys.path.append('/data/Disk_D/shaochuan/Project/DUNET-master/nips_deploy')
...
model_name = 'DUNET_defence_AdaAtt'
...
def track_video(model, video, net1, gpu_str):
    image_save = 0
    toc, regions = 0, []
        ...
        if f == 0:  # init
            ...
            if args.purify_adv:
                if f % 30 == 1:  # clean the perturbation from last frame
                    ...
                    def_per = 0
                    # statistic the convergence of the loss 10.1
                    state, att_per, def_per, attack_loss = DUNET_pure_adv(state, z_feat, im, f, regions[f - 1],
                                                                           att_per, def_per,
                                                                           image_save, net1, gpu_str, iter=10)

                    # state, att_per, def_per, attack_loss = DAA_DUNET(state, z_feat, im, f, regions[f - 1],
                    #                                                       att_per, def_per,
                    #                                                       image_save, net1, gpu_str, iter=10)
                    ...
                    state, att_per, def_per, attack_loss = DUNET_pure_adv(state, z_feat, im, f, regions[f - 1],
                                                                           att_per,
                                                                           def_per,
                                                                           image_save, net1, gpu_str, iter=5)
                ...
                state = DUNET_pure_clean(state, im, f, regions[f - 1], net1, gpu_str)
                location = cxy_wh_2_rect(state['target_pos'] + 1, state['target_sz'])
                regions.append(location)
...
def main():
    ...
    # new added 5.8  depoly DUNET
    config, resmodel = get_model1()
    net1 = resmodel.net
    checkpoint = torch.load('/data/Disk_D/shaochuan/Project/DUNET-master/nips_deploy/denoise_res_015.ckpt')
    if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
        resmodel.load_state_dict(checkpoint['state_dict'])
    else:
        resmodel.load_state_dict(checkpoint)
    resmodel = resmodel.to(gpu_str)
    resmodel.eval()
    # new added end
    ...
    if args.sequence is not None:
        video = args.sequence
        track_video(net, dataset[video], net1, gpu_str)
    else:
        for v_id, video in enumerate(dataset.keys()):
            if v_id > -1:
                fps_list.append(track_video(net, dataset[video], net1, gpu_str))
