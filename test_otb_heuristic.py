...
from run_attack import SiamRPN_init, heuristic_pure_adv, heuristic_pure_clean, DAA_heuristic
...
def track_video(model, video, gpu_str):
    image_save = 0
    toc, regions = 0, []
        ...
        if f == 0:  # init
            ...
            if args.purify_adv:
                if f % 30 == 1:  # clean the perturbation from last frame
                    ...
                    def_per = 0
                    # statistic the convergence of the loss 10.1
                    state, att_per, def_per, attack_loss = heuristic_pure_adv(state, z_feat, im, f, regions[f - 1],
                                                                           att_per, def_per,
                                                                           image_save, method, gpu_str, iter=10)
                    # state, att_per, def_per, attack_loss = DAA_heuristic(state, z_feat, im, f, regions[f - 1],
                    #                                                           att_per, def_per,
                    #                                                           image_save, method, gpu_str, iter=10)
                    ...
                    state, att_per, def_per, attack_loss = heuristic_pure_adv(state, z_feat, im, f, regions[f - 1],
                                                                           att_per,
                                                                           def_per,
                                                                           image_save, method, gpu_str, iter=5)
                    # state, att_per, def_per, attack_loss = DAA_heuristic(state, z_feat, im, f, regions[f - 1],
                    #                                                           att_per,
                    #                                                           def_per,
                    #                                                           image_save, method, gpu_str, iter=5)
                ...
                state = heuristic_pure_clean(state, im, f, regions[f - 1], method, gpu_str)
                location = cxy_wh_2_rect(state['target_pos'] + 1, state['target_sz'])
                regions.append(location)
...
def main():
    ...
    if args.sequence is not None:
        video = args.sequence
        track_video(net, dataset[video], gpu_str)
    else:
        for v_id, video in enumerate(dataset.keys()):
            if v_id > -1:
                fps_list.append(track_video(net, dataset[video], gpu_str))
